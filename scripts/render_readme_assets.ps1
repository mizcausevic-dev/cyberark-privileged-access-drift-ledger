...
d.text((96,106),'CYBERARK PRIVILEGED ACCESS DRIFT LEDGER',font=small,fill=green)
d.text((96,178),'Privileged access drift becomes visible',font=serif,fill=text)
d.text((96,238),'before audit becomes board exposure.',font=serif,fill=text)
d.text((96,332),'Vaulting, rotation, checkout exceptions, break-glass review,',font=body,fill=muted)
d.text((96,372),'stale owners, and session recording resolve into one PAM posture.',font=body,fill=muted)
for i,(label,val) in enumerate([('AGGREGATE DRIFT','56.6'),('ESCALATION LANES','1'),('EXPOSED ACCOUNTS','9'),('TOP LANE','domain admin')]):
    x=96+i*272
...
im=Image.new('RGB',(W,H),bg); d=ImageDraw.Draw(im)
d.text((64,70),'Privileged access lanes',font=serif,fill=text)
card(d,(64,150),(360,430),pink,'ESCALATE',['domain admin','safe'],['Rotation age and','checkout exceptions','need remediation.'],'100.0')
card(d,(464,150),(360,430),violet,'WATCH',['cloud root','access'],['Break-glass review','needs executive-ready','evidence.'],'55.3')
card(d,(864,150),(360,430),green,'CONTAINED',['database DBA','safe'],['Vaulting and session','proof are usable for','quarterly review.'],'15.6')
